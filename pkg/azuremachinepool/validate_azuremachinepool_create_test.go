...
func TestAzureMachinePoolCreateValidate(t *testing.T) {
	...
	for i, instanceType := range unsupportedInstanceType {
		testCases = append(testCases, testCase{
			...
			nodePool:     azureMPRawObject(instanceType, &tr, string(compute.StorageAccountTypesStandardLRS), []capzv1alpha3.DataDisk{}),
			allowed:      false,
		...
		testCases = append(testCases, testCase{
			...
			nodePool:     azureMPRawObject(instanceType, &fa, string(compute.StorageAccountTypesStandardLRS), []capzv1alpha3.DataDisk{}),
			allowed:      true,
			errorMatcher: nil,
		})
		...
		testCases = append(testCases, testCase{
			...
			nodePool:     azureMPRawObject(instanceType, nil, string(compute.StorageAccountTypesStandardLRS), []capzv1alpha3.DataDisk{}),
			allowed:      true,
			errorMatcher: nil,
		})
	}
