...
func TestAzureMachinePoolCreateValidate(t *testing.T) {
	...
	for i, instanceType := range unsupportedInstanceType {
		testCases = append(testCases, testCase{
			...
			nodePool:     azureMPRawObject(instanceType, &tr, string(compute.StorageAccountTypesStandardLRS), desiredDataDisks, "westeurope", nil),
		...
		testCases = append(testCases, testCase{
			...
			nodePool:     azureMPRawObject(instanceType, &fa, string(compute.StorageAccountTypesStandardLRS), desiredDataDisks, "westeurope", nil),
			errorMatcher: nil,
		})

		testCases = append(testCases, testCase{
			name:         fmt.Sprintf("case %d: instance type %s with accelerated networking nil", i*3+2, instanceType),
			nodePool:     azureMPRawObject(instanceType, nil, string(compute.StorageAccountTypesStandardLRS), desiredDataDisks, "westeurope", nil),
			errorMatcher: nil,
		})
	}
	...
	testCases = append(testCases, testCase{
		...
		nodePool:     azureMPRawObject("Standard_A2_v2", nil, string(compute.StorageAccountTypesStandardLRS), desiredDataDisks, "eastgalicia", nil),
		errorMatcher: IsInvalidOperationError,
	})
