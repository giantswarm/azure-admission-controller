...
func TestAzureMachinePoolCreateValidate(t *testing.T) {
	...
	for i, instanceType := range unsupportedInstanceType {
		testCases = append(testCases, testCase{
			...
			nodePool:     azureMPRawObject(instanceType, &tr, compute.StorageAccountTypesStandardLRS),
			allowed:      false,
		...
		testCases = append(testCases, testCase{
			...
			nodePool:     azureMPRawObject(instanceType, &fa, compute.StorageAccountTypesStandardLRS),
			allowed:      true,
			errorMatcher: nil,
		})
		...
		testCases = append(testCases, testCase{
			...
			nodePool:     azureMPRawObject(instanceType, nil, compute.StorageAccountTypesStandardLRS),
			allowed:      true,
			errorMatcher: nil,
		})
	}
