...
func (m *UpdateMutator) Mutate(ctx context.Context, request *v1beta1.AdmissionRequest) ([]mutator.PatchOperation, error) {
	...
	patch, err := mutator.EnsureComponentVersionLabelFromRelease(ctx, m.ctrlClient, m.logger, clusterCR.GetObjectMeta(), "azure-operator", label.AzureOperatorVersion)
	if err != nil {
		return []mutator.PatchOperation{}, microerror.Mask(err)
	}
	if patch != nil {
	...
	patch, err = mutator.EnsureComponentVersionLabelFromRelease(ctx, m.ctrlClient, m.logger, clusterCR.GetObjectMeta(), "cluster-operator", label.ClusterOperatorVersion)
	if err != nil {
		return []mutator.PatchOperation{}, microerror.Mask(err)
	}
	if patch != nil {
